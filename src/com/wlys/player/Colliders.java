...
public final class Colliders
{
    ...
    public static <V extends GameObject, W extends GameObject, T extends V, U extends W> void collide(
            T go, Array<U> gos, ColliderHandler<V, W> callback)
    {
    ...
    public static <U extends GameObject, T extends U> void collide(Array<T> a,
            ColliderHandler<U, U> callback)
    {
    ...
    public static <V extends GameObject, W extends GameObject, T extends V, U extends W> void collide(
            Array<T> a, Array<U> b, ColliderHandler<V, W> callback)
    {
        if (a != b)
        {
            for (int i = a.size - 1; i >= 0; i--)
    ...
    public static <U extends GameObject, T extends U> void collide(
            Array<T> gos, Array<Rectangle> rects, SceneryHandler<U> callback)
    {
        for (int i = gos.size - 1; i >= 0; i--)
        {
            T go = gos.get(i);
    ...
    public static <U extends GameObject, T extends U> void collide(T go,
            Array<Rectangle> rects, SceneryHandler<U> callback)
    {
        for (int i = 0; i < rects.size && !go.isInCollision(); i++)
        {
     ...
     */
    public static <T extends GameObject, U extends T> void removeMarkedCollisions(
            Pool<U> pool, Array<U> gos, RemovalHandler<T> handler)
    {
        ...
        for (int i = gos.size - 1; i >= 0; i--)
        {
            ...
            if (go.isDemandRemove())
            {
                ...
                gos.removeIndex(i);
                pool.free(go);
                logger.info("É¾³ýÅö×²¶ÔÏó£º"+go.toString());
            }
        }
    }
     ...
     */
    public static <T extends GameObject> void removeOutOfBounds(Pool<T> pool,
            Array<T> gos, Rectangle bounds)
    {
        float minX = bounds.x;
        ...
        {
            T go = gos.get(i);
            if (go.getX() >= maxX || go.getX() + go.getWidth() <= minX
                    || go.getY() >= maxY || go.getY() + go.getHeight() <= minY)
            {
                gos.removeIndex(i);
                pool.free(go);
                logger.info("³¬³öÆÁÄ»É¾³ý¶ÔÏó£º"+go.toString());
            }
        }
    }
}
